...
engine = sqlalchemy.create_engine(
  f"postgresql://{b_owner}:{b_password}@localhost:5437/{base_name}"
  )
...
def people_info(data: list):
  ...
  insert = f"""INSERT INTO search_result(user_id, owner_id, first_name, last_name)
...
def liked_photos(owner_id: int, data: list):
  if len(data) != 3:
    ...
    likes_count_photo_1, link_photo_2,
    likes_count_photo_2, link_photo_3,
    likes_count_photo_3)
    VALUES ({owner_id},'{link_photo_1}',
    {likes_count_photo_1}, '{link_photo_2}',
    {likes_count_photo_2}, '{link_photo_3}',
    {likes_count_photo_3})"""
    connection.execute(insert)
...
def output_search_result(number_of_candidates):
    insert = f"""SELECT sd.owner_id, first_name, last_name,
    link_photo_1, link_photo_2, link_photo_3
    FROM sorted_data sd
...
def add_favorite(owner_id: int, data: list):
  connection.execute(
    f"""INSERT INTO favorites (owner_id, photo_link)
...
def ban_list(owner_id: int):
  connection.execute(
    f"""INSERT INTO ban_list (owner_id)
        VALUES ({owner_id})"""
    )
...
def clear_database():
  connection.execute(
    """TRUNCATE sorted_data;
      TRUNCATE search_result;
      TRUNCATE main_user;"""
    )
